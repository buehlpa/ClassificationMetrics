...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      menuItem("Analysis", tabName = "analysisTab", icon = icon("chart-line"))
    ),
    ...
    sliderInput("mean2", "Mean of Cluster 2", min = 0, max = 10, value = 5),
    sliderInput("sd1", "Standard Deviation of Cluster 1", min = 0.1, max = 2, value = 1),
    sliderInput("sd2", "Standard Deviation of Cluster 2", min = 0.1, max = 2, value = 1)
  ),
  dashboardBody(
    tabItems(
...
server <- function(input, output) {
  ...
  output$precisionRecallPlot <- renderPlotly({
    ...
    plot_ly(x = recall_values,
            y = precision_values,
            type = 'scatter', mode = 'lines+markers',text = ~paste('Threshold:', round(thresholds, 2)),) %>%
